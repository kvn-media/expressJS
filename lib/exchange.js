...
function createExchange(exchangeData) {
    ...
    BUY: BUY,
    SELL: SELL,
    buy: function (prices, volume, exchangeData) {
        return order(BUY, price, volume, exchangeData);
    },
    sell: function (prices, volume, exchangeData) {
    ...
    getDisplay: function(exchangeData) {
        ...
        if (sells) {
            for (var prices in sells.volumes) {
        ...
        while (sellPrices.size() > 0) {
            var sellPrice = sellPrice.pop()
            stringBook +=
              padding + sellPrice + ", " + sells.volumes[sellPrice] + "\n";
        }
        while (buyPrices.size() > 0) {
            var buyPrices = buyPrices.pop();
            stringBook += buyPrices + ", " + buys.volumes[buyPrices] + "\n";
        }
        stringBook += "\n\n";
